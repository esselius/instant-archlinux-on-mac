...
eval "$(docker-machine env --shell bash docker-vm)"
###############################################################################
...
docker run \
  ...
  bash -c "run-remote-script https://raw.githubusercontent.com/esselius/instant-archlinux-on-mac/master/mac-install-internal.sh"
